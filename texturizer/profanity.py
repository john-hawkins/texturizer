    ...
    def prof_features(x, col):
        ...
        if x[col]!=x[col]:
            ...
            if set(slur_list).intersection(word_array):
                contains_slur = 1
        return hard_profanity, mild_profanity, non_profanity, contains_insult, contains_slur

    df[[ col+'_profane_hard', col+'_profane_mild', col+'_profane_non', col+'_insult', col+'_slur']] = df.apply(prof_features, col=col, axis=1, result_type="expand")
    return df
